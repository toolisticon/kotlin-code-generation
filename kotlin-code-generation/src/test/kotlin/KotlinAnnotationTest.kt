...
internal class KotlinAnnotationTest {
  ...
  fun `create Foo annotation`() {
    ...
    assertThat(spec.code).isEqualTo("""@io.toolisticon.kotlin.generation.KotlinAnnotationTest.Foo(bar = "hello world", x = "foo", type = kotlin.Long::class)""")
    assertThat(spec.members).hasSize(3)

    assertThat(spec).hasToString(
      "KotlinAnnotationSpec(" +
        "typeName=io.toolisticon.kotlin.generation.KotlinAnnotationTest.Foo, " +
        "members=[" +
        "bar = \"hello world\", " +
        "x = \"foo\", " +
